...
class Scraper
{
                        ...
                        if($node->filter($val['selector'])->count() > 0) {
                            ...
                            if ($val['is_attribute'] == false) {
                                ...
                                if ($key == 'source_link') {
                                    ...
                                    // append website url in case the Movie is not full url
    ...
    protected function save($data)
    {
        foreach ($data['title'] as $k => $val) {
            ...
            $checkExist = Movie::where('source_link', $data['source_link'][$k])->first();
            ...
            if(!isset($checkExist->id)) {
                ...
                $Movie->genre = isset($data['genre'][$k]) ? $data['genre'][$k] : "";

                $Movie->duration = isset($data['duration'][$k]) ? $data['duration'][$k] : "";

                $Movie->actor = isset($data['actor'][$k]) ? $data['actor'][$k] : "";

                $Movie->synopsis = isset($data['synopsis'][$k]) ? $data['synopsis'][$k] : "";
                
                $Movie->trailer = isset($data['trailer'][$k]) ? $data['trailer'][$k] : "";

                $Movie->rating = isset($data['rating'][$k]) ? $data['rating'][$k] : "";

                $Movie->review = isset($data['review'][$k]) ? $data['review'][$k] : "";

                $Movie->poster = isset($data['poster'][$k]) ? $data['poster'][$k] : "";

                $Movie->source_link = $data['source_link'][$k];

                $Movie->category_id = $data['category_id'][$k];

                $Movie->website_id = $data['website_id'][$k];
